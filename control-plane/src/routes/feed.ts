...
import { getDbForOrg, getDb } from "../db/client";
...
feedRoutes.openapi(promotePostRoute, async (c): Promise<any> => {
  ...
  const [post] = await sql`SELECT id, org_id FROM feed_posts WHERE id = ${post_id} AND org_id = ${user.org_id}`;
  ...
  const deducted = await sql`
    ...
    WHERE org_id = ${user.org_id} AND balance_usd >= ${costUsd}
  `;
