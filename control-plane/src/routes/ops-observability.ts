...
export const opsObservabilityRoutes = new Hono<R>();

// ---------------------------------------------------------------------------
// 1. GET /agents/:name/health — Agent health check
// ---------------------------------------------------------------------------
opsObservabilityRoutes.get(
  "/agents/:name/health",
  requireScope("observability:read"),
  async (c) => {
    const user = c.get("user");
    const agentName = c.req.param("name");
    const windowMinutes = Math.max(1, Math.min(1440, Number(c.req.query("window_minutes")) || 30));
...
opsObservabilityRoutes.get(
  ...
  async (c) => {
    const user = c.get("user");
    const windowMinutes = Math.max(1, Math.min(1440, Number(c.req.query("window_minutes")) || 60));
    const since = new Date(Date.now() - windowMinutes * 60 * 1000).toISOString();
    const sql = await getDbForOrg(c.env.HYPERDRIVE, user.org_id);
