...
async function callLLM(
  ...
  const model = config.model || env.DEFAULT_MODEL || "@cf/meta/llama-3.1-70b-instruct";
...
export class AgentOSWorker extends Agent<Env, AgentState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname.endsWith("/run")) {
        const body = (await request.json()) as { input: string; config?: Partial<AgentConfig> };
        if (body.config) {
          this.setState({
            ...this.state,
      ...
      if (url.pathname.endsWith("/memory")) {
        const episodes = [...this.sql<Episode>`SELECT * FROM episodes ORDER BY timestamp DESC LIMIT 20`];
        const procedures = [...this.sql<Procedure>`SELECT * FROM procedures ORDER BY last_used DESC LIMIT 20`];
        return jsonResponse({
      ...
      if (request.method === "POST" && url.pathname.endsWith("/memory/working")) {
      ...
      if (request.method === "POST" && url.pathname.endsWith("/ingest")) {
        const body = (await request.json()) as { documents: { id: string; text: string; metadata?: Record<string, string> }[] };
      ...
      if (request.method === "POST" && url.pathname.endsWith("/eval")) {
        const body = (await request.json()) as { tasks: EvalTask[]; trialsPerTask?: number };
        const report = await this.runEval(body.tasks, body.trialsPerTask || 3);
        return jsonResponse(report);
      }

      // GET /eval/report
      if (url.pathname.endsWith("/eval/report")) {
        const results = [...this.sql<EvalTrialResult>`SELECT * FROM eval_results ORDER BY created_at DESC LIMIT 100`];
        return jsonResponse(results);
      }
      ...
      if (request.method === "PUT" && url.pathname.endsWith("/config")) {
        const updates = (await request.json()) as Partial<AgentConfig>;
        this.setState({
          ...this.state,
  ...
  private async buildMemoryContext(query: string): Promise<string> {
    ...
    if (words.length > 0) {
      const like = `%${words[0]}%`;
      const episodes = [
        ...this.sql<Episode>`SELECT * FROM episodes WHERE input LIKE ${like} OR output LIKE ${like} ORDER BY timestamp DESC LIMIT 3`,
      ];
    ...
    const procedures = [
      ...this.sql<Procedure>`SELECT * FROM procedures ORDER BY success_count DESC LIMIT 3`,
    ];
  ...
  private async storeEpisode(input: string, output: string): Promise<void> {
    ...
    this.sql`INSERT INTO episodes (id, input, output, timestamp, outcome)
  ...
  private async storeProcedure(
    ...
    const existing = [...this.sql<Procedure>`SELECT * FROM procedures WHERE name = ${name}`];
    if (existing.length > 0) {
      if (success) {
        this.sql`UPDATE procedures SET success_count = success_count + 1, last_used = ${Date.now()} WHERE name = ${name}`;
      } else {
        this.sql`UPDATE procedures SET failure_count = failure_count + 1, last_used = ${Date.now()} WHERE name = ${name}`;
      }
    } else {
      this.sql`INSERT INTO procedures (name, steps, description, success_count, failure_count, last_used)
  ...
  private async runEval(
    ...
    for (const task of tasks) {
      for (let trial = 1; trial <= trialsPerTask; trial++) {
        ...
        // Persist
        this.sql`INSERT INTO eval_results (task_name, trial, passed, score, latency_ms, output, created_at)
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/" || url.pathname.startsWith("/assets")) {
      return env.ASSETS
        ? (env as unknown as { ASSETS: Fetcher }).ASSETS.fetch(request)
        : new Response("AgentOS API is running. Use /agents/agentos/:name for agent access.", {
